...
async function uploadDashboardData(event) {
    ...
    if (!file) {
        statusDiv.innerHTML = '<div style="color: #dc3545;">Please select a file</div>';
        return;
    }
    ...
    submitBtn.disabled = true;
    statusDiv.innerHTML = '<div style="color: #007aff;">Reading file...</div>';
    ...
    try {
        ...
        reader.onload = async function(e) {
            ...
            statusDiv.innerHTML = '<div style="color: #007aff;">Uploading to database...</div>';
            
            // Call database function to upload data
            const { data, error } = await supabase.rpc('upload_new_dashboard_data', {
                p_data_content: csvContent
            });
            ...
            statusDiv.innerHTML = '<div style="background: #d4edda; color: #155724; padding: 12px; border-radius: 6px;">✅ Data uploaded successfully! Dashboard will update automatically.</div>';
            
            // Reset form
            fileInput.value = '';
            
            // Reload dashboard iframe if present
        ...
        statusDiv.innerHTML = `<div style="background: #f8d7da; color: #721c24; padding: 12px; border-radius: 6px;">❌ Error: ${error.message}</div>`;
    } finally {
        submitBtn.textContent = originalText;
        submitBtn.disabled = false;
    }
